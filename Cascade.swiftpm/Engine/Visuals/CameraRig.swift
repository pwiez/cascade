...
/// An orbit camera: a pivot at Earth's centre that rotates, and a camera that
/// slides along the pivot's local Z to zoom.
///
/// Splitting rotation and distance across two entities is what keeps the maths
/// trivial — neither ever has to account for the other.
@MainActor
final class CameraRig {
    ...
    /// Stops just short of the poles, where an orbit camera gimbal-locks and the
    /// horizon flips.
    private let pitchRange: ClosedRange<Float> = -1.4...1.4

    /// Vertical FOV as a fraction of distance, used to convert a fraction of the
    /// screen into a world-space offset. Matches the 52° camera below.
    ...
    /// Slides the camera sideways so a panel covering part of the screen doesn't
    /// cover Earth with it. `ratio` is a fraction of the viewport width.
